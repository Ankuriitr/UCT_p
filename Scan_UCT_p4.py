...
gr_time=np.zeros(NR)    # To store graphic timing info.
trans_loop_time=np.zeros(NR)
for rot in range(0,NR):
    ...
    for dis in range(0,NT):
        ...
        with open(filename,'w') as f:                           # To save data to text file.
            np.savetxt(f,data)
            f.close()
        proj_data[dis,rot]=proj_dtrnd(0,N_packets,data,0)      # Generate the projection data based on CLT.
        i=i+1
    ...
    reconst_data=reconstruct(NR,NT,NT,proj_data_all,3)    # It take projection data in which rotations are stored in column.
    
    graphic_start_time=time.time()
    ...
    plt.show()
    plt.close()
    graphic_end_time=time.time()
    gr_time[rot]=graphic_end_time-graphic_start_time
...
# Saving timeing data.
trans_loop_time_file='E:\\Ankur\\P4_docs\\Time_analysis\\Python\\Measurement_only\\W_pro\\trans_loop_time'+str(N_rot)+'x'+str(N_translation)+'.dat'
with open(trans_loop_time_file,'w') as f:                           # To save data to text file.
    np.savetxt(f,trans_loop_time)
    f.close()
    
time_grphcs_file='E:\\Ankur\\P4_docs\\Time_analysis\\Python\\Measurement_only\\W_pro\\graph_time'+str(N_rot)+'x'+str(N_translation)+'.dat'
with open(time_grphcs_file,'w') as f:                           # To save data to text file.
    np.savetxt(f,gr_time)
    f.close()

time_code_parts_file='E:\\Ankur\\P4_docs\\Time_analysis\\Python\\Measurement_only\\W_pro\\code_parts_time'+str(N_rot)+'x'+str(N_translation)+'.dat'
